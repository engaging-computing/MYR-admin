...
const ColorPalletField = (props)=>{
    const { source, defaultValue="#000000" } = props;
    const record = useRecordContext(props);
    //using lodash's get to iterate through the source's path
    const color = get(record,source);
    return <Paper elevation={2} sx={{
        backgroundColor: color ? color : defaultValue,
        width:100,
        height:20
    }}/>;
}
        ...
        <GridShowLayout>
            <RaGrid container>
                <RaGrid xs={6}>
                    ...
                    <CodeDisplayField source="code" readOnly={true}/>
                </RaGrid> 
                <RaGrid xs={6}>
